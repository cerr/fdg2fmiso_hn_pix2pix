...
if __name__ == '__main__':
    ...
    mae_file_name = os.path.join(checkpoints_dir, 'test_tumorwise_corr_log.txt')
    with open(mae_file_name, 'a') as file:
        now = time.strftime("%c")
        file.write('==========mean of test tumorwise corr  (Training at %s)==========\n' % now)

    # current_mae = float('inf')
    current_corr = float('-inf')
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        opt.eval = True  #use eval mode in dev testing
        ...
        running_corr = 0
        # get test filenames
        filename_test = [os.path.basename(x) for x in dataset_test.dataset.image_paths]
        patientName_test = [x.split('_')[1] if len(x.split('_')) ==3 else '_'.join(x.split('_')[1:3]) for x in filename_test]
        patientNameUnique_test = list(set(patientName_test))
        patientNameUnique_test.sort()
        def index_contain_substring(list, substr):
            for i, s in enumerate(list):
                if substr in s:
                    return i
            return -1
        patient_test_id_list = [index_contain_substring(filename_test, x) for x in patientNameUnique_test] #index of first slice of each test tumor

        for i in range(len(patient_test_id_list)):
            y_target_tumor = []
            y_predict_tumor = []
            if i == len(patient_test_id_list) - 1:
                j_end = len(dataset_test)
            else:
                j_end = patient_test_id_list[i+1]
            for j in range(patient_test_id_list[i], j_end):
                data = dataset_test.dataset[j]
                data['A'] = torch.unsqueeze(data['A'], 0)
                data['B'] = torch.unsqueeze(data['B'], 0)
                ...
                #get tumor mask, use -1 as threshold as 0 is transfered to -1
                mask = torch.unsqueeze(visuals['real_A'][:,1,:,:]>-1, 1)
                y_target_tumor.append(y_target[mask].cpu())
                y_predict_tumor.append(y_predict[mask].cpu())

                if j % (patient_test_id_list[i] + j_end)//2 == 0 and epoch % 10 == 1:
                    name = os.path.basename(data['B_paths']).split('.')[0]
                    for label, img_tensor in visuals.items():
                        ...
                        if img_tensor.size(dim=1) == 2:
                            ...
                            img1_name = '%s_%s_epoch%d_1.png' % (name, label, epoch)
                            img2_name = '%s_%s_epoch%d_2.png' % (name, label, epoch)
                            save_path1 = os.path.join(checkpoints_dir,'train_' + label, img1_name)
                            save_path2 = os.path.join(checkpoints_dir,'train_' + label, img2_name)
            ...
            corri, _ = stats.spearmanr(torch.cat(y_target_tumor), torch.cat(y_predict_tumor))
            running_corr += corri
        ...
        #     #get tumor mask, use -1 as threshold as 0 is transfered to -1
        #     mask = torch.unsqueeze(visuals['real_A'][:,1,:,:]>-1, 1)
        #     y_target_tumor = y_target[mask].cpu()
        #     y_predict_tumor = y_predict[mask].cpu()
        #     # error = torch.abs(y_predict_tumor - y_target_tumor).sum()/y_target_tumor.numel()
        #     # running_mae += error
        #     corri, _ = stats.spearmanr(y_target_tumor, y_predict_tumor)
        #     running_corr += corri
        ...
        corr = running_corr/len(patient_test_id_list)
        message = '%d, %.6f' % (epoch, corr)
        with open(mae_file_name, 'a') as file:
            file.write('%s\n' % message)

        if corr > current_corr:
            current_corr = corr
