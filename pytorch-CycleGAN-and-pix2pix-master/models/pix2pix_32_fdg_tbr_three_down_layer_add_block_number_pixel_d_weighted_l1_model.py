...
from . import networks_three_down_layer_add_block_number
from .weighted_l1 import WeightedL1Loss

class Pix2Pix32FdgTbrThreeDownLayerAddBlockNumberPixelDWeightedL1Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='instance', netG='unet_32', dataset_mode='aligned', netD='pixel')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A', 'fake_B', 'real_B', 'k1']
        ...
        self.netG = networks_three_down_layer_add_block_number.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks_three_down_layer_add_block_number.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:
            # define loss functions
            self.criterionGAN = networks_three_down_layer_add_block_number.GANLoss(opt.gan_mode).to(self.device)
    ...
    def set_input(self, input):
        ...
        self.k1 = input['k1'].to(self.device)
