...
from . import networks_add_block_number


class Pix2Pix32TbrThreeDownLayerAddBlockNumberModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks_add_block_number.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks_add_block_number.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:
            # define loss functions
            self.criterionGAN = networks_add_block_number.GANLoss(opt.gan_mode).to(self.device)
