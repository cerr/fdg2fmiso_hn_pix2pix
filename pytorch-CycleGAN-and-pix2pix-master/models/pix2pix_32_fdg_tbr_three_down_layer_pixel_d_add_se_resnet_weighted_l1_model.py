...
from . import networks_three_down_layer_add_se_resnet
from .weighted_l1 import WeightedL1Loss

class Pix2Pix32FdgTbrThreeDownLayerPixelDAddSeResnetWeightedL1Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks_three_down_layer_add_se_resnet.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks_three_down_layer_add_se_resnet.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:
            # define loss functions
            self.criterionGAN = networks_three_down_layer_add_se_resnet.GANLoss(opt.gan_mode).to(self.device)
