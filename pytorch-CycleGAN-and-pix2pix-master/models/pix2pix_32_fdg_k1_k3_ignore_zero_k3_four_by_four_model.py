...
from . import networks_four_by_four


class Pix2Pix32FdgK1K3IgnoreZeroK3FourByFourModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='instance', netG='unet_32', dataset_mode='aligned')
    ...
    def __init__(self, opt):
        ...
        self.netG = networks_four_by_four.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks_four_by_four.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                           opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:
            # define loss functions
            self.criterionGAN = networks_four_by_four.GANLoss(opt.gan_mode).to(self.device)
    ...
    def backward_G(self):
        ...
        # calcuate the l1 and l2 loss within tumor region
        # 0 transfered to -1, therefore use -1 for throholding tumor region
        # ignore the 0 k1 voxel when calculating L1, L2 loss
