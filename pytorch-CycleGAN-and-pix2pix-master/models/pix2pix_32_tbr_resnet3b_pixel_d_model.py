...
from . import networks_resnet3b


class Pix2Pix32TbrResnet3bPixelDModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='instance', netG='resnet_3blocks', dataset_mode='aligned', netD='pixel', ngf=32, ndf=32)
    ...
    def __init__(self, opt):
        ...
        self.netG = networks_resnet3b.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            self.netD = networks_resnet3b.define_D(opt.input_nc + opt.output_nc, opt.ndf, opt.netD,
                                          opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)

        if self.isTrain:
            # define loss functions
            self.criterionGAN = networks_resnet3b.GANLoss(opt.gan_mode).to(self.device)
