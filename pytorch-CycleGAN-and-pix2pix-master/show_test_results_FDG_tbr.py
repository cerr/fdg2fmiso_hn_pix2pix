...
if __name__ == '__main__':
    ...
    opt.results_dir = os.path.join(opt.checkpoints_dir, 'best_model_test_result_image')
    if not os.path.isdir(opt.results_dir):
        os.mkdir(opt.results_dir)
    if not os.path.isdir(os.path.join(opt.checkpoints_dir, 'best_model_test_result_mat')):
        os.mkdir(os.path.join(opt.checkpoints_dir, 'best_model_test_result_mat'))
    ...
    for i, data in enumerate(dataset):
        ...
        with open(mae_file_name, 'a') as file:
            file.write('%s\n' % message)
        data_dict = {name + '_real_A' : real_A_tensor.cpu().numpy(), name + '_real_B' : real_B_tensor.cpu().numpy(), name + '_fake_B': fake_B_tensor.cpu().numpy(), name + '_k1': visuals['k1'].cpu().numpy()}
        savemat(opt.checkpoints_dir + '/best_model_test_result_mat/' + name + '.mat', data_dict)
