...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(dataset_test):
            ...
            #ignore k3<0.0025 voxels, tranfered to -0.9905677
            mask = torch.logical_and(torch.unsqueeze(visuals['real_A'][:,1,:,:]>-1, 1), y_target>-0.9905677)
            y_target_tumor = y_target[mask]
            y_predict_tumor = y_predict[mask]
            if y_target_tumor.numel() == 0:
                error = 0
            else:
