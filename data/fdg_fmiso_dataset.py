...
class FdgFmisoDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.no_augmentation:
            ...
            apply_augmentation = transforms.Compose(
                [transforms.RandomCrop(self.opt.crop_size, padding=(self.opt.load_size - self.opt.crop_size)//2)])
        ...
        data_B = img_tensor_return[self.input_nc:,:,:]
        
        #this is to facilitate the pix2pix model.py    

        return {'A': data_A, 'B': data_B, 'A_paths': AB_path, 'B_paths': AB_path}
